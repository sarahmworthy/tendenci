...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Looping through forms_formentry records
        and updating field_function names.
        """
        # the FormEntry class that comes via orm
        # does not contain custom methods
        from tendenci.apps.forms_builder.forms.models import FormEntry

        # only get fields with "field_functions"
        form_fields = orm.Field.objects.filter(field_function__isnull=False)

        field = {
            'EmailFirstName': 'first_name',
            'EmailLastName': 'last_name',
            'EmailPhoneNumber': 'phone_number',
            'GroupSubscription': 'group_subscription',
        }

        # update field_function names
        for form_field in form_fields:
            new_field = field.get(form_field.field_function, u'')
            if new_field:
        ...
        # flatten data into formentry table
        for form_entry in form_entries:
            form_entry.first_name = form_entry.get('first_name')
            form_entry.last_name = form_entry.get('last_name')
            form_entry.email = form_entry.get('email')
            form_entry.phone = form_entry.get('phone_number')
